...
describe('Work Center UI contract', () => {
  ...
  it('uses progressive Work Item and Action workspaces with messages, lazy request detail, and input', () => {
    ...
    expect(detail).toContain('class="work-center-action-transcript"');
    expect(detail).toContain('class="work-center-action-view-switch"');
    expect(detail).toContain('class="work-center-action-context"');
    expect(detail).toContain('class="work-center-action-execution"');
    expect(detail).not.toContain('class="work-center-action-inspector"');
    expect(detail).toContain('v-for="generation in actionThread"');
    expect(detail).toContain('v-for="message in generation.messages"');
    expect(detail).toContain('executorName');
    ...
    expect(detail).toContain('workCenter.actionConversation');
    expect(detail).toContain('workCenter.actionContext');
    expect(detail).toContain("activeView: 'conversation'");
    expect(detail).toContain("setActiveView('execution')");
    expect(css).toContain('width: min(100%, 1120px)');
    expect(css).toContain('@container work-center (max-width: 760px)');
    expect(css).not.toContain('grid-template-columns: minmax(0, 1fr) minmax(250px, 320px)');
    ...
    expect(detail).toContain("$t('workCenter.actionChatPlaceholder'");
    expect(detail).toContain('<strong>{{ executorName }}</strong>');
  ...
  it('uses breadcrumbs, close semantics, and active-first Action projection', () => {
    ...
    expect(page).not.toContain('class="work-center-action-breadcrumb"');
    expect(page).not.toContain("$t('workCenter.actionNumber', { number: actionSequence(selectedAction) })");
    expect(page).not.toContain('actionBreadcrumbDescription(selectedAction)');
