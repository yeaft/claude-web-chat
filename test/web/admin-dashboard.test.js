...
// =====================================================================
// 10. Mobile responsive layout (≤640px)
// =====================================================================
describe('mobile responsive layout', () => {
  it('CSS has media query for max-width 640px', () => {
    expect(dashboardCss).toContain('@media (max-width: 640px)');
  });

  it('desktop table is hidden on mobile', () => {
  ...
  it('card list is shown on mobile', () => {
    const mobileSection = dashboardCss.split('@media (max-width: 640px)')[1] || '';
    expect(mobileSection).toContain('.db-card-list');
    expect(mobileSection).toMatch(/\.db-card-list\s*\{[^}]*display:\s*flex/);
  });

  it('card list is hidden on desktop', () => {
    // Before the media query, card-list should be display: none
    const desktopSection = dashboardCss.split('@media')[0] || '';
    expect(desktopSection).toContain('.db-card-list');
    expect(desktopSection).toMatch(/\.db-card-list\s*\{[^}]*display:\s*none/);
  });

  it('stats row uses grid layout on mobile', () => {
    const mobileSection = dashboardCss.split('@media (max-width: 640px)')[1] || '';
    expect(mobileSection).toContain('.db-stats-row');
    expect(mobileSection).toContain('grid-template-columns');
  });

  it('template has mobile card variants for user stats', () => {
    expect(dashboardSource).toContain('db-user-card');
    expect(dashboardSource).toContain('db-user-card-name');
    expect(dashboardSource).toContain('db-user-card-stats');
    expect(dashboardSource).toContain('db-user-card-meta');
  });

  it('template has mobile card variants for agents', () => {
    expect(dashboardSource).toContain('db-agent-card');
    expect(dashboardSource).toContain('db-agent-card-name');
    expect(dashboardSource).toContain('db-agent-card-stats');
    expect(dashboardSource).toContain('db-agent-card-meta');
  });

  it('template has mobile card variants for online users', () => {
    expect(dashboardSource).toContain('db-online-card');
    expect(dashboardSource).toContain('db-online-card-name');
  });

  it('mobile cards use unique keys with m- prefix', () => {
    expect(dashboardSource).toContain("'m-' + user.username");
    expect(dashboardSource).toContain("'m-' + agent.name");
  });

  it('CSS has card styling for mobile cards', () => {
    const mobileSection = dashboardCss.split('@media (max-width: 640px)')[1] || '';
...
// =====================================================================
// 12. CSS — db-* naming and key rules
// =====================================================================
describe('CSS naming and key rules', () => {
  it('all CSS classes use db- prefix (except shared ones)', () => {
    // Extract class selectors from dashboard.css — must start with a letter after the dot
    const classSelectors = dashboardCss.match(/\.[a-zA-Z][\w-]*/g) || [];
    const dashboardClasses = classSelectors.filter(c =>
      !c.startsWith('.is-') && !c.startsWith('.online') && !c.startsWith('.offline') && !c.startsWith('.sp-')
    );
    dashboardClasses.forEach(cls => {
      expect(cls).toMatch(/^\.db-/);
    });
  });

  it('has loading spinner with CSS animation', () => {
    expect(dashboardCss).toContain('.db-loading::before');
    expect(dashboardCss).toContain('animation: spin');
  });

  it('has latency color classes for good/warn/bad', () => {
    expect(dashboardCss).toContain('.db-latency-good');
    expect(dashboardCss).toContain('.db-latency-warn');
    expect(dashboardCss).toContain('.db-latency-bad');
  });

  it('latency-good uses success color', () => {
    const goodLine = dashboardCss.split('.db-latency-good')[1]?.split('}')[0] || '';
    expect(goodLine).toContain('var(--success)');
  });

  it('latency-bad uses error color', () => {
    const badLine = dashboardCss.split('.db-latency-bad')[1]?.split('}')[0] || '';
    expect(badLine).toContain('var(--error)');
  });

  it('status dot has online and offline variants', () => {
    expect(dashboardCss).toContain('.db-status-dot.online');
    expect(dashboardCss).toContain('.db-status-dot.offline');
  });

  it('table uses tabular-nums for numeric cells', () => {
    expect(dashboardCss).toContain('.db-cell-num');
    expect(dashboardCss).toContain('tabular-nums');
  });

  it('table rows have hover effect', () => {
    expect(dashboardCss).toContain('.db-table tbody tr:hover');
  });

  it('table last row removes border-bottom', () => {
    expect(dashboardCss).toContain('.db-table tbody tr:last-child td');
    expect(dashboardCss).toContain('border-bottom: none');
  });

  it('stat value has is-active color variant', () => {
    expect(dashboardCss).toContain('.db-stat-value.is-active');
    const activeSection = dashboardCss.split('.db-stat-value.is-active')[1]?.split('}')[0] || '';
    expect(activeSection).toContain('var(--success)');
  });
});
...
describe('CSS integration', () => {
  it('dashboard.css is included in loadAllCss output', () => {
    expect(cssSource).toContain('.db-stats-row');
    expect(cssSource).toContain('.db-table');
    expect(cssSource).toContain('.db-refresh-btn');
  });

  it('index.css imports dashboard.css', () => {
    const indexCss = readFileSync(resolve(__dirname, '../../web/styles/index.css'), 'utf-8');
    expect(indexCss).toContain("@import './dashboard.css'");
  });
});
