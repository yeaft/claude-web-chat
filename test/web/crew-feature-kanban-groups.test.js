...
describe('structural integrity', () => {
  it('CSS has balanced braces (2094/2094)', () => {
  ...
  it('agent test brace count synchronized to 2094', () => {
    const agentTestPath = resolve(__dirname, '../../test/agent/crew.test.js');
    const agentTestSource = readFileSync(agentTestPath, 'utf-8');
    expect(agentTestSource).toContain('expect(opens).toBe(2094)');
  });

  it('crew-remove-maxwidth test brace count synchronized to 2094', () => {
    const rmTestPath = resolve(__dirname, '../../test/web/crew-remove-maxwidth.test.js');
    const rmTestSource = readFileSync(rmTestPath, 'utf-8');
    expect(rmTestSource).toContain('expect(opens).toBe(2094)');
  });

  it('crew-scroll-to-role test brace count synchronized to 2094', () => {
    ...
    expect(scrollTestSource).toContain('expect(opens).toBe(2094)');
  });
});
