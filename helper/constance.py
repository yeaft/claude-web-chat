...
# 获取 MongoDB 主机地址，优先使用环境变量 MONGODB_HOST
MONGODB_HOST = os.getenv("MONGODB_HOST", "localhost")
MONGODB_CONNECTION_STRING = f"mongodb://{MONGODB_HOST}:27017"
...
FINAL_RESULT_COL = MongoClient(
    MONGODB_CONNECTION_STRING).future.finalVerifyResult
MERGED_FILTERS_COL = MongoClient(
    MONGODB_CONNECTION_STRING).future.mergedFilters
...
CANDLE_FACTOR_EXPECT_COL = MongoClient(
    MONGODB_CONNECTION_STRING).future.candleFactorExpectBest
CANDLE_FACTOR_WIN_RATE_COL = MongoClient(
    MONGODB_CONNECTION_STRING).future.candleFactorWinRateBest
...
REAL_TIME_TICK_SECOND_COL = MongoClient(
    MONGODB_CONNECTION_STRING).future.realTimeTickSecond
TRAIN_OPT_COL = MongoClient(MONGODB_CONNECTION_STRING).future.trainOperation
TRAIN_COL = MongoClient(MONGODB_CONNECTION_STRING).future.trainStatus
...
SIMILAR_FACTOR_COL = MongoClient(
    MONGODB_CONNECTION_STRING).future.similarFactor
TRADE_INFO_COL = MongoClient(MONGODB_CONNECTION_STRING).future.tradeInfo
TRADE_INFO_HISTORY_COL = MongoClient(
    MONGODB_CONNECTION_STRING).future.tradeInfoHistory
...
NIGHT_TRADE_TYPES = ["a", "ag", "al", "au", "b", "bu", "c", "cf", "cs", "cu", "cy", "eb", "eg", "fg", "fu", "hc", "i", "j", "jm", "l", "m",
                     "ma", "ni", "nr", "oi", "p", "pb", "pg", "pp", "rb", "rm", "rr", "ru", "sa", "sc", "sn", "sp", "sr", "ss", "ta", "v", "y", "zc", "zn"]
CARE_TYPES = ["sf", "sm", "i", "ru", "ap", "ma", "zc", "p", "pp", "wr", "jm", "hc", "l", "fg", "rm", "bu"]
# CARE_TYPES = ["sf", "sm", "i", "ru", "ap", "ma", "zc", "pp", "wr", "fb", "jm", "hc", "l", "fg", "rm", "bu"]
LARGE_DIFF_TYPES = ["fu", "i", "bu", "ma", "ni", "p", "jd", "nr", "ap",
                    "j", "ru", "sc", "wr", "y", "cf", "rm", "b", "rb", "hc", "cj", "ta", "jd"]
