  ...
  template: `
    <div class="crew-chat-view">
      ...
      <div class="crew-messages" ref="messagesRef" @scroll="onScroll">
        ...
        <!-- Active Messages: latest message from each role, always visible -->
...
你负责竞争分析、博弈推演、战略规划和风险评估。`
  ...
  computed: {
    ...
    activeMessages() {
      // Collect the latest text message from each non-human role (persistent, not just streaming)
      const messages = this.store.currentCrewMessages;
      const result = [];
      const seen = new Set();
      for (let i = messages.length - 1; i >= 0; i--) {
        const m = messages[i];
        if (m.type === 'text' && m.role && m.role !== 'human' && !seen.has(m.role)) {
          seen.add(m.role);
          result.push(m);
        }
      }
