/**
 * rolePlayInput.js — Input composable for RolePlay mode.
 *
 * Reuses crewInput's UI features (@-mention, slash commands, file upload)
 * but sends via store.sendMessage() (normal conversation) instead of sendCrewMessage().
 */
...
export function createRolePlayInput(store, authStore, { getInputRef, getFileInputRef, getCurrentPendingAsk }) {
  ...
  // Role list from rolePlaySession
  ...
  const slashGroupedCommands = Vue.computed(() => {
    ...
    const groups = [];
    if (system.length > 0) groups.push({ label: 'System', items: system, isLast: project.length === 0 });
    if (project.length > 0) groups.push({ label: 'Project', items: project, isLast: true });
    return groups;
  });
  ...
  function handleKeydown(e, sendMessage) {
    if (slashMenuVisible.value && slashFilteredCommands.value.length > 0) {
      if (e.key === 'ArrowDown') { e.preventDefault(); slashMenuIndex.value = (slashMenuIndex.value + 1) % slashFilteredCommands.value.length; return; }
      if (e.key === 'ArrowUp') { e.preventDefault(); slashMenuIndex.value = (slashMenuIndex.value - 1 + slashFilteredCommands.value.length) % slashFilteredCommands.value.length; return; }
      if (e.key === 'Tab' || (e.key === 'Enter' && !e.shiftKey)) { e.preventDefault(); selectSlashCommand(slashFilteredCommands.value[slashMenuIndex.value]); return; }
      if (e.key === 'Escape') { e.preventDefault(); slashMenuVisible.value = false; return; }
    }
    if (atMenuVisible.value && filteredAtRoles.value.length > 0) {
      if (e.key === 'ArrowDown') { e.preventDefault(); atMenuIndex.value = (atMenuIndex.value + 1) % filteredAtRoles.value.length; return; }
      if (e.key === 'ArrowUp') { e.preventDefault(); atMenuIndex.value = (atMenuIndex.value - 1 + filteredAtRoles.value.length) % filteredAtRoles.value.length; return; }
      if (e.key === 'Enter' || e.key === 'Tab') { e.preventDefault(); selectAtRole(filteredAtRoles.value[atMenuIndex.value]); return; }
      if (e.key === 'Escape') { e.preventDefault(); atMenuVisible.value = false; return; }
    }
    if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(); }
  }

  function onBlur() {
    setTimeout(() => { slashMenuVisible.value = false; }, 150);
  }
  ...
  /**
   * Send message via standard store.sendMessage() — the RolePlay way.
   * Unlike Crew which uses sendCrewMessage().
   */
  function sendMessage(e, scrollToBottom) {
    ...
    if (scrollToBottom) Vue.nextTick(() => scrollToBottom());
  }

  function saveDraft(convId) {
    if (convId && inputText.value) store.inputDrafts[convId] = inputText.value;
  }
  ...
  return {
    inputText, attachments, uploading,
    atMenuVisible, atQuery, atMenuIndex,
    canSend, filteredAtRoles,
    slashMenuVisible, slashMenuIndex,
    slashFlatItems, slashGroupedCommands, slashFilteredCommands,
    selectSlashCommand, onBlur,
    handleInput, selectAtRole, handleKeydown,
    handlePaste, handleFileSelect, addFiles,
    removeAttachment, sendMessage,
