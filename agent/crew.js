...
# Feature: {taskTitle}
- task-id: {task-id}
- 状态: 待开发 | 开发中 | 待审查 | 审查中 | 待测试 | 测试中 | 已完成 | 已阻塞
- 负责人: {dev角色name}
- 审查者: {reviewer角色name}
- 测试者: {tester角色name}
- 创建时间: {ISO时间}
...
3. dev 完成 → 更新为「待审查」+「待测试」，记录改动摘要
4. reviewer 审查 → 更新为「审查中」，完成后记录审查结论
5. tester 测试 → 更新为「测试中」，完成后记录测试结果
6. 全部通过 → PM 更新为「已完成」

## 角色职责
- **PM**: 创建 feature 文件，填写需求描述，最终标记完成
...
- TASKS 块不需要在回复最末尾，可以放在任意位置`;
  ...
  if (role.isDecisionMaker) {
    prompt += `\n\n# Feature 进度文件管理
当你分配任务时，必须同时创建 feature 进度文件：
1. 在 \`context/features/\` 目录下创建 \`{task-id}.md\`（如 \`context/features/task-1.md\`）
2. 使用共享 CLAUDE.md 中定义的文件格式，填写需求描述
3. 初始状态设为「待开发」
4. 所有子任务完成后，将状态更新为「已完成」

注意：feature 文件的创建和更新通过 ROUTE 给 developer 执行（因为你不能使用 Write/Edit 工具）。在 ROUTE 的 summary 中明确要求 dev 创建/更新 feature 文件。`;
