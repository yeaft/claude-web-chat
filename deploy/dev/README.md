...
2. 复制 `deployer.env.example` 为 `deployer.env`，填写主机路径和 Docker network。dev 保持 `BROWSER_RUNTIME_ENABLED=true`，与 Server 默认行为一致；这只开放 Browser setup/viewer 协议，不会把 Chrome 加入 Server/Agent 镜像，也不会触发下载。Chrome 仍由用户在所选 Agent 的 Workbench 中点击“启用浏览器”后按 instance 安装。远程 Viewer 还需要可达的 ICE 路径；跨 NAT 环境必须按 [`../browser-turn/`](../browser-turn/README.md) 部署 TURN，并把相同 shared secret 写入 `WEBCHAT_ENV_FILE` 后滚动替换 Server。可将该值设为 `false` 全局关闭 Browser。
